...
`esy-python` is sample package, [`hello`](https://github.com/esy/test-hello-c), packaged for [`esy`](https://esy.sh/).
...
## How to use `esy-python`?

`esy-python` can be used from both NPM and directly from Github.

### From NPM 

`esy-python` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-python` to install it, or specify it in
...
{
  "dependencies": {
    "esy-python": "esy-packages/esy-python"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-python": "esy-packages/esy-python#<commit hash>"
  }
```
